...
/**
 * =====================================================================
 * CHIMYON SCHOOL — FOYDALANUVCHI XABARLARI (admin/messages.php)
 * ---------------------------------------------------------------------
 * Sayt orqali kelgan xabarlarni ko'rish, o'qilgan deb belgilash va o'chirish.
 * =====================================================================
 */
...
try {
    // Faqat o'qilmaganlar yoki barchasi
    $filter = $_GET['filter'] ?? 'all';
    
    if ($filter === 'unread') {
        $where = "oqilgan = 0";
        $count_sql = "SELECT COUNT(*) FROM messages WHERE oqilgan = 0";
        $sql = "SELECT * FROM messages WHERE oqilgan = 0 ORDER BY sana DESC LIMIT $limit OFFSET $offset";
    } else {
        $count_sql = "SELECT COUNT(*) FROM messages";
        $sql = "SELECT * FROM messages ORDER BY sana DESC LIMIT $limit OFFSET $offset";
    }
...
if (isset($_POST['mark_read']) && isset($_POST['id'])) {
    try {
        $stmt = db()->prepare("UPDATE messages SET oqilgan = 1 WHERE id = ?");
        $stmt->execute([$_POST['id']]);
        header('Location: messages.php?marked=1');
        exit;
    } catch (Throwable $ex) {
        $error = "Belgilashda xatolik: " . $ex->getMessage();
    }
}
...
$xabar_bildirish = '';
if (isset($_GET['marked'])) $xabar_bildirish = "Xabar o'qilgan deb belgilandi!";
if (isset($_GET['deleted'])) $xabar_bildirish = "Xabar o'chirildi!";
...
<body>
    ...
    <main class="main">
        ...
        <div class="card">
            <div class="filters">
                <a href="?filter=all" class="<?= $filter === 'all' ? 'active' : '' ?>">Barchasi</a>
                <a href="?filter=unread" class="<?= $filter === 'unread' ? 'active' : '' ?>">O'qilmaganlar</a>
            </div>
            ...
            <?php if ($xabarlar): ?>
                <table>
                    ...
                    <tbody>
                        <?php foreach ($xabarlar as $x): ?>
                            <tr class="<?= $x['oqilgan'] == 0 ? 'unread' : '' ?>">
                                <td><?= (int)$x['id'] ?></td>
                                <td><?= e($x['ism']) ?></td>
                                <td><?= e($x['aloqa']) ?></td>
                                <td><?= e(mb_substr($x['xabar'], 0, 60)) ?><?= mb_strlen($x['xabar']) > 60 ? '...' : '' ?></td>
                                <td>
                                    <?php if ($x['oqilgan'] == 0): ?>
                                        <span class="badge badge-new">Yangi</span>
                                    <?php else: ?>
                                        <span class="badge badge-old">O'qilgan</span>
                                    <?php endif; ?>
                                </td>
                                <td><?= date('d.m.Y H:i', strtotime($x['sana'])) ?></td>
                                <td>
                                    <?php if ($x['oqilgan'] == 0): ?>
                                        <form method="post" style="display:inline;">
                                            <input type="hidden" name="id" value="<?= (int)$x['id'] ?>">
                                            <button type="submit" name="mark_read" class="btn btn-read">O'qilgan</button>
                                        </form>
                                    <?php endif; ?>
                                    <a href="?delete=<?= (int)$x['id'] ?>" class="btn btn-danger" onclick="return confirm('Rostdan ham o\'chirmoqchimisiz?')">O'chirish</a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
